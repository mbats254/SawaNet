...
class AdminController extends Controller
{
    ...
    public function upgrade_package_post(Request $request)
    {
     ...
     {
         $admin_type = Auth::user()->admin->status;
         if($admin_type !== 1)
         {
            Log::info("Unauthorized Entry");
            $request->session()->flash("error", "Unauthorized Entry");
            return redirect()->back();
         }
         else {
     ...
     public function post_new_admin(Request $request)
     {
        $request->validate([
            'email' => 'required|string|email|max:255|unique:users',
            'name' => 'required|string|max:255',
        ...
        $user->notify(new WelcomeAdmin($user));
        Log::info($user->name ." Added Successfully");
        $request->session()->flash("success", $user->name ." Added Successfully");
     ...
     public function all_admins(Request $request)
     {
         ...
         return view('admin.all_admins',compact('admin'));
     }
     ...
     public function remove_admin(Request $request)
     {
         ...
         $user->update(['role' => 'ex-admin']);
         Log::info("Admin Successfully Removed");
         $request->session()->flash("success", "Admin Successfully Removed");
          return redirect()->back();
     }
}
